...
//        SerialFd serialFd = new SerialFd();
        ...
        string[] PortName = new string[10];// = "No recognized";
                ...
                case (int)SET_COMMANDS.SET_COMMPORT:
                    ...
                    */
                    ...
                    foreach (ManagementObject property in Ports)
                    {
                        if (property.GetPropertyValue("Name") != null)
                            if (//property.GetPropertyValue("Name").ToString().Contains("XDS110") &&
                                property.GetPropertyValue("Name").ToString().Contains("COM1") ||
                                property.GetPropertyValue("Name").ToString().Contains("COM2") ||
                                property.GetPropertyValue("Name").ToString().Contains("COM3") ||
                                property.GetPropertyValue("Name").ToString().Contains("COM4") ||
                                property.GetPropertyValue("Name").ToString().Contains("COM5") ||
                                property.GetPropertyValue("Name").ToString().Contains("COM6") ||
                                property.GetPropertyValue("Name").ToString().Contains("COM7") ||
                                property.GetPropertyValue("Name").ToString().Contains("COM8") ||
                                property.GetPropertyValue("Name").ToString().Contains("COM9"))
                               // property.GetPropertyValue("Name").ToString().Contains("XDS110 Class Application/User UART"))
                            {
                                ...
                                device[deviceCnt++] = property.GetPropertyValue("Name").ToString();
                              //  break;
                            }
                    }
                    ...
                    foreach (string port in ports)
                    {
                        ...
                        {
                            if (device[portNbr].Contains(ports[j]))
                            {
                                PortName[portNbr] = ports[j];
                                break;
                            }
                        }

                        //ComportBox.Items.Add(port + " " + device[portNbr]);
                        ComportBox.Items.Add(device[portNbr]);
                        ComportBox.SelectedIndex = portNbr;
                        //PortName[portNbr] = port;

                        if (Regex.IsMatch(device[portNbr], @matchStr))
                        {
