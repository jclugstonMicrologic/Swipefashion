...
//return 0;
  ...
  //while ((erasestatus == FLASH_COMPLETE) && (sectorindex <= FLASH_Sector_9) ) //FLASH_Sector_11) )//&& (HCD_IsDeviceConnected(&USB_OTG_Core) == 1))
  while ((erasestatus == FLASH_COMPLETE) && (sectorindex <=(startsector)))//128)) )// FLASH_Sector_11) ) //FLASH_Sector_11) )//&& (HCD_IsDeviceConnected(&USB_OTG_Core) == 1))
  {
    erasestatus = FLASH_EraseSector(sectorindex, VoltageRange_3);
    sectorindex +=1;// 8;
...
//  if ((sectorindex != (FLASH_Sector_9 + 8)) || (erasestatus != FLASH_COMPLETE))    
  if ((sectorindex != (FLASH_Sector_9 + 1)) || (erasestatus != FLASH_COMPLETE))    
  {
    return 1;
  }
  else
...
/**
  ...
  */
FLASH_Status FLASH_If_ProgramWord(uint32_t Address, uint32_t *pData)
{
  FLASH_Status status = FLASH_COMPLETE;

  status = FLASH_ProgramWord(Address, *(uint32_t *)(&pData));
  return status;
}
